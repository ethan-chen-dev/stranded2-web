...
- 子项目 4 物品使用与合成建造：手持物品、徒手与刀类近战攻击（`attack1`/`hit`/`impact`/`kill` 事件流、`find=` 掉落、`loot=`、武器状态）、右键工具动作（锤子建造、铲子挖掘、鱼竿钓鱼）、背包多选合成（`combinations*.inf`、相似分组、锁）、建筑菜单与工地建造（`buildings.inf`、放置约束、逐件投料、`build`/`build_finish` 事件、锁）。远程与投掷武器留到子项目 5。

后续子项目：单位 AI 与战斗（含远程武器与投射物）、战役与序列、随机岛与编辑器。
